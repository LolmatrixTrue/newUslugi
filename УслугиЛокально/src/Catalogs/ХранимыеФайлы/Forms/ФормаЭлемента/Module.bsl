...
Процедура ПоместитьМультимедиа(ДанныеМультимедиа)
	...
	Если ДанныеМультимедиа <> Неопределено Тогда
		...
		АдресВременногоХранилища = ПоместитьВоВременноеХранилище(ДанныеМультимедиа.ПолучитьДвоичныеДанные(),
...
Процедура Подписать(Команда)  
	...
	Оповещение = Новый ОписаниеОповещения("ПодписатьПослеПодключенияРасширения", ЭтотОбъект);
...
Процедура ПодписатьПослеПодключенияРасширения(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Данные.Количество() = 0 Тогда
		ПоказатьПредупреждение( , НСтр("ru = 'Нет данных файла!!!'", "ru"), 10);
		Возврат;
	КонецЕсли;
...
Процедура ПодписатьПослеПолученияСпискаСертификатов(Результат, Контекст) Экспорт
	...
	Контекст2 = Новый Структура("МенеджерКриптографии, ФайлДвоичныеДанные, Сертификат, Данные", МенеджерКриптографии,
		Контекст.ФайлДвоичныеДанные, Результат, Контекст.Данные);

	Оповещение = Новый ОписаниеОповещения("ПодписатьПослеСозданияМенеджераКриптографии", ЭтотОбъект, Контекст2);
...
Процедура ПолучитьСледующиеПодписи(СертификатыПодписи, Контекст) Экспорт
	...
	Если СертификатыПодписи <> Неопределено Тогда
		Для Каждого СертификатПодписи Из СертификатыПодписи Цикл
			Если Контекст.Сертификат = СертификатПодписи Тогда
				ПоказатьПредупреждение( , НСтр("ru = 'Этим сертификатом файл уже подписан'", "ru"), 10);
				Возврат;
			КонецЕсли;
		КонецЦикла;
	...
	Если Контекст.ПодписиПолучены.Количество() = Контекст.Данные.Количество() Тогда
		...
		// подписываем
		Контекст2 = Новый Структура("МенеджерКриптографии, ФайлДвоичныеДанные, Сертификат, Данные",
	...
	Оповещение = Новый ОписаниеОповещения("ПолучитьСледующиеПодписи", ЭтотОбъект, Контекст);
...
Процедура ПодписатьЗавершение2(Результат, Контекст) Экспорт
	Если Результат = КодВозвратаДиалога.ОК Тогда
		...
		Оповещение = Новый ОписаниеОповещения("ПослеПодписиФайла", ЭтотОбъект, Контекст);
...
Процедура ПроверитьПодпись(Команда)
	...
	Если ПроверитьПодписьНаСервере() Тогда
		...
		ПоказатьПредупреждение( , Сообщение, 3);
	КонецЕсли;
КонецПроцедуры
...
Процедура ПоместитьЗашифрованныйНаСервер(Команда)
	...
	Оповещение = Новый ОписаниеОповещения("ПоместитьЗашифрованныйПослеПодключенияРасширенияКриптографии", ЭтотОбъект);
	НачатьПодключениеРасширенияРаботыСКриптографией(Оповещение);
...
Процедура ПоместитьЗашифрованныйПослеПодключенияРасширенияКриптографии(Результат, ДополнительныеПараметры) Экспорт
	...
	Оповещение = Новый ОписаниеОповещения("ПоместитьЗашифрованныйПослеИнициализацииМенеджера", ЭтотОбъект);
	МенеджерКриптографии.НачатьИнициализацию(Оповещение, "", "", 75);
...
Процедура ПоместитьЗашифрованныйПослеИнициализацииМенеджера(МенеджерКриптографии, ДополнительныеПараметры) Экспорт
	...
	Контекст = Новый Структура("МенеджерКриптографии, НовыйОбъект, ИсходныеДанныеДляШифрования", МенеджерКриптографии,
		Объект.Ссылка.Пустая(), Неопределено);

	Оповещение = Новый ОписаниеОповещения("ПоместитьЗашифрованныйПослеПодключенияРасширенияФайлов", ЭтотОбъект,
		Контекст);
...
Процедура ПоместитьЗашифрованныйПослеПодключенияРасширенияФайлов(Результат, Контекст) Экспорт
	...
	Если Результат Тогда
		...
		Оповещение = Новый ОписаниеОповещения("ПоместитьЗашифрованныйПослеВыбораФайлаВДиалоге", ЭтотОбъект, Контекст);
		Диалог.Показать(Оповещение);

	Иначе
		Оповещение = Новый ОписаниеОповещения("ПоместитьЗашифрованныйПослеПомещенияБезРасширения", ЭтотОбъект, Контекст);
...
Процедура ПоместитьЗашифрованныйНаСерверЗавершение(Результат, Контекст) Экспорт
	...
	Оповещение = Новый ОписаниеОповещения("ПоместитьЗашифрованныйПослеШифрования", ЭтотОбъект, Контекст);
...
Процедура Зашифровать(Команда)
	...
	Оповещение = Новый ОписаниеОповещения("ЗашифроватьПослеПодключенияРасширения", ЭтотОбъект);
	НачатьПодключениеРасширенияРаботыСКриптографией(Оповещение);
...
Процедура ЗашифроватьЗавершение(Результат, Контекст) Экспорт
	...
	Контекст = Новый Структура("НовыйОбъект, Сертификаты, ДанныеСертификатов", НовыйОбъект, Сертификаты, Новый Массив);
...
Процедура ВыгрузитьСледующийСертификатДляШифрования(ВыгруженныеДанные, Контекст) Экспорт
	...
	Оповещение = Новый ОписаниеОповещения("ВыгрузитьСледующийСертификатДляШифрования", ЭтотОбъект, Контекст);
...
Процедура ПолучитьСРасшифровкой(Команда)
	...
	Оповещение = Новый ОписаниеОповещения("ПолучитьСРасшифровкойПослеПодключенияРасширения", ЭтотОбъект);
	НачатьПодключениеРасширенияРаботыСКриптографией(Оповещение);
...
Процедура ПолучитьСРасшифровкойПослеПодключенияРасширения(Результат, ДополнительныеПараметры) Экспорт
	...
	МенеджерКриптографии = Новый МенеджерКриптографии;
	Контекст = Новый Структура("МенеджерКриптографии, ФайлДвоичныеДанные", МенеджерКриптографии, ФайлДвоичныеДанные);

	Оповещение = Новый ОписаниеОповещения("ПолучитьСРасшифровкойПослеИнициализацииМенеджера", ЭтотОбъект, Контекст);
	МенеджерКриптографии.НачатьИнициализацию(Оповещение, "", "", 75);
...
Процедура ПолучитьСРасшифровкойЗавершение(Результат, Контекст) Экспорт
	...
	Оповещение = Новый ОписаниеОповещения("ПолучитьСРасшифровкойЗавершениеПодклчюениярасширенияФайлов", ЭтотОбъект,
		Контекст);
	НачатьПодключениеРасширенияРаботыСФайлами(Оповещение);
...
Процедура ПолучитьСРасшифровкойЗавершениеПодклчюениярасширенияФайлов(Результат, Контекст) Экспорт
	...
	Если Результат Тогда
		...
		Оповещение = Новый ОписаниеОповещения("ПолучитьСРасшифровкойПослеВыбораФайла", ЭтотОбъект, Контекст);
		Диалог.Показать(Оповещение);

	Иначе

		Оповещение = Новый ОписаниеОповещения("ПолучитьСРасшифровкойПослеРасшифровкиБезрасширения", ЭтотОбъект,
			Контекст);
...
Процедура ПолучитьСРасшифровкойПослеВыбораФайла(ВыбранныеФайлы, Контекст) Экспорт
	...
	Оповещение = Новый ОписаниеОповещения("ПолучитьСРасшифровкойПослеРасшифровкиСРасширением", ЭтотОбъект, Контекст);
	Контекст.МенеджерКриптографии.НачатьРасшифровку(
...
// МножественныйВыбор
&НаКлиенте
Процедура ПолучитьСписокСертификатов(ПараметрыВыбора, МножественныйВыбор, ИмяПроцедурыЗавершения,
	...
	ФормаСпискаСертификатов.ОписаниеОповещенияОЗакрытии = Новый ОписаниеОповещения(ИмяПроцедурыЗавершения, ЭтотОбъект,
		Контекст);
...
Процедура ВводПароля(МенеджерСертификатов, ИмяПроцедурыЗавершения, Контекст = Неопределено)
	...
	ФормаПароля.ОписаниеОповещенияОЗакрытии = Новый ОписаниеОповещения(ИмяПроцедурыЗавершения, ЭтотОбъект, Контекст);
	ФормаПароля.Открыть();
КонецПроцедуры
...
Функция ПолучитьИмяФайла(ПолныйПутьКФайлу)
	...
	ПозицияРазделителя = СтрНайти(ПолныйПутьКФайлу, ПолучитьРазделительПути(), НаправлениеПоиска.СКонца);

	Если ПозицияРазделителя = 0 Тогда
		Возврат ПолныйПутьКФайлу;
	КонецЕсли;

	Возврат Прав(ПолныйПутьКФайлу, СтрДлина(ПолныйПутьКФайлу) - ПозицияРазделителя);

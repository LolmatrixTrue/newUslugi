...
Функция ПолучитьЗначениеВладельца()
	...
	Для Каждого Элемент Из Список.Отбор.Элементы Цикл
		...
		Если ТипЗнч(Элемент) = Тип("ЭлементОтбораКомпоновкиДанных") И (Строка(Элемент.ЛевоеЗначение) = "Владелец"
			Или Строка(Элемент.ЛевоеЗначение) = "Owner") И Элемент.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно Тогда
...
Процедура ЗагрузитьФайлыПослеПодключенияРасширения(Подключено, Параметры) Экспорт
	...
	Иначе
		...
		ПослеПомещенияФайлов = Новый ОписаниеОповещения("ПослеПомещенияФайлов", ЭтотОбъект, ДопПараметры,
			"ПриОшибкеПомещения", ЭтотОбъект);
		...
		НачатьПомещениеФайловНаСервер(ПослеПомещенияФайлов, ОХодеПомещенияФайлов, ПередПомещенияФайлов,
			ПараметрыДиалога);
	КонецЕсли;
...
Процедура ЗагрузитьФайлыЗавершение(Результат, Параметры) Экспорт
	Если Не Результат = Неопределено Тогда
		...
		Для Каждого ЗагруженныйФайл Из Результат.СписокЗагруженныхФайлов Цикл
			ДанныеФайла = Новый Структура("ПолноеИмяФайла, АдресВХранилище", ЗагруженныйФайл.Значение.СсылкаНаФайл.Имя,
				ЗагруженныйФайл.Значение.Адрес);
...
Процедура ОткрытьФайл()
	...
	Если ПередаваемыеФайлы.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	ПослеПодключенияРасширения = Новый ОписаниеОповещения("ОткрытьФайл_ПослеПодключенияРасширения", ЭтотОбъект,
		ПередаваемыеФайлы);
	НачатьПодключениеРасширенияРаботыСФайлами(ПослеПодключенияРасширения);
...
#Если Не МобильныйКлиент Тогда
	...
	КонецЕсли;
#Иначе
		ПослеПолучениеКаталогаВременныхФайлов = Новый ОписаниеОповещения("ПолучениеКаталогаВременныхФайлов",
			ЭтотОбъект, ПередаваемыеФайлы);
...
#КонецЕсли
	...
	ПослеЗапросаРазрешенийПользователя = Новый ОписаниеОповещения("ПослеЗапросаРазрешенийПользователя", ЭтотОбъект,
		ПередаваемыеФайлы);
...
Процедура ПослеПолученияФайлов(ПереданныеФайлы, ДопПараметры) Экспорт
	Если Не ПереданныеФайлы = Неопределено Тогда
		Для Каждого Описание Из ПереданныеФайлы Цикл
			ПослеЗапускаПриложения = Новый ОписаниеОповещения("ПослеЗапускаПриложения", ЭтотОбъект, Описание.ПолноеИмя);
...
&НаКлиенте
Процедура ХодПомещенияФайлов(ПомещаемыйФайл, Помещено, ОтказОтПомещенияФайла, ПомещеноВсего,
	ОтказОтПомещенияВсехФайлов, ДопПараметры) Экспорт
